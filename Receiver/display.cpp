...
void drawBatteryScreen(Adafruit_SSD1306& display) {
  int batX = 10, batY = 18, batW = 50, batH = 28;

  // Draw Battery Outline
  display.drawRect(batX, batY, batW, batH, SSD1306_WHITE);
  display.fillRect(batX + batW, batY + 8, 4, 12, SSD1306_WHITE); // Terminal
  ...
  if (tank.dataValid) {
    ...
    // BIG percentage on the right side
    display.setTextSize(3);
    int percentX = 75;  // Right side of screen
    // Adjust position based on digit count
    if (tank.batteryPercent < 10) {
      percentX = 85;  // Single digit - shift right
    } else if (tank.batteryPercent >= 100) {
      percentX = 68;  // 100% - shift left
    }
    display.setCursor(percentX, 18);
    ...
    display.setCursor(batX + 15, batY + 8);
    display.print("?");

    // No data text on right
    display.setTextSize(2);
...
void drawStatsScreen(Adafruit_SSD1306& display) {
  ...
  display.print("D:");  // Shortened from "Dist:" to save space
  if (tank.dataValid) {
