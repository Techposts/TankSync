...
void startWiFiConnectionNonBlocking() {
  ...
  Serial.printf("▶ WiFi Attempt #%d: %s (non-blocking)\n", wifiRetryCount, savedSSID.c_str());
...
void updateWiFiConnection() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    wifiRetryCount = 0;
    WiFi.setSleep(false);  // CRITICAL: Disable WiFi sleep to prevent WDT crashes
    Serial.printf("  ✓ Connected! IP: %s (took %lu ms)\n", WiFi.localIP().toString().c_str(), elapsed);
  }
  else if (elapsed > WIFI_CONNECT_TIMEOUT) {
    Serial.printf("  ✗ Failed after %lu ms (Attempt #%d)\n", elapsed, wifiRetryCount);
    Serial.println("  → Switching to AP Mode...");
    startAPMode();
  }
}
...
void startAPMode() {
  ...
  if (WiFi.softAP(AP_SSID, AP_PASSWORD, AP_CHANNEL, false, AP_MAX_CONNECTIONS)) {
    ...
    Serial.printf("  ✓ AP Active: %s @ 192.168.4.1\n", AP_SSID);

    if (savedSSID.length() > 0) {
      Serial.println("  ℹ Will retry WiFi every 5 minutes");
    } else {
      Serial.println("  ℹ No WiFi configured - AP mode only");
    }
  } else {
...
void checkWiFiStatus() {
  ...
  if (wifiState == WIFI_STATE_CONNECTED && WiFi.status() != WL_CONNECTED) {
    ...
    if (savedSSID.length() > 0) {
      Serial.println("  → Attempting to reconnect (non-blocking)...");
  ...
  if (wifiState == WIFI_STATE_AP_MODE && savedSSID.length() > 0) {
    ...
    unsigned long remaining = WIFI_RETRY_INTERVAL - elapsed;

    // FIXED: Only show countdown message once per minute
    static unsigned long lastWiFiRetryMessage = 0;
    if (millis() - lastWiFiRetryMessage > 60000) {
      lastWiFiRetryMessage = millis();
      int minsLeft = remaining / 60000;
      if (minsLeft > 0) {
        Serial.printf("ℹ WiFi retry in %d minutes...\n", minsLeft);
      }
    }

    // Time to retry
    if (elapsed > WIFI_RETRY_INTERVAL) {
      Serial.println("\n⏰ 5 minutes elapsed - retrying WiFi connection...");
      WiFi.softAPdisconnect(true);
      delay(500);  // Increased delay to allow AP to shut down cleanly
      yield();
      startWiFiConnectionNonBlocking();
    }
  }
...
void handleScan(WebServer& server) {
  ...
  Serial.printf("  Current WiFi mode: %d (AP=%d, STA=%d, APSTA=%d)\n",
                WiFi.getMode(), WIFI_MODE_AP, WIFI_MODE_STA, WIFI_MODE_APSTA);
  yield();
  ...
  if (n == WIFI_SCAN_FAILED) {
    Serial.println("  ✗ Scan failed (WIFI_SCAN_FAILED)");
    n = 0;
  } else {
  ...
  for (int i = 0; i < n && networksAdded < 15; i++) {
    ...
    Serial.printf("    %d: %s (%d dBm)\n", networksAdded + 1, ssid.c_str(), WiFi.RSSI(i));
    networksAdded++;
    yield();
  }
  j += "]}";
