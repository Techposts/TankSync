    ...
    for (;;) {
        ...
        switch (s_current_effect) {
            ...
            case LED_EFFECT_BLINK_GREEN:
                ...
                vTaskDelay(pdMS_TO_TICKS(250));
                break;
            case LED_EFFECT_BLINK_BLUE:
                for(int i=0; i<s_num_leds; i++) s_leds[i] = (step % 2) ? LED_BLUE : LED_OFF;
                led_show();
                vTaskDelay(pdMS_TO_TICKS(300));
                break;
            case LED_EFFECT_PULSE_CYAN: {
                float val = 0.55f + 0.45f * sinf(step * 0.08f);
                uint8_t old_br = s_brightness;
                s_brightness = (uint8_t)(old_br * val);
                for(int i=0; i<s_num_leds; i++) s_leds[i] = LED_CYAN;
                led_show();
                s_brightness = old_br;
                vTaskDelay(pdMS_TO_TICKS(30));
                break;
