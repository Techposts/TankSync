...
// Driver health: 2.6.2 — strip can disconnect/reconnect at runtime; never permanent-disable
static bool        s_initialized = false;
static bool        s_strip_present = false;
static int         s_rmt_fail_count = 0;
static TickType_t  s_last_fail_tick = 0;
#define LED_FAIL_BACKOFF_THRESHOLD 5
#define LED_RETRY_INTERVAL_TICKS pdMS_TO_TICKS(5000)
...
esp_err_t led_init(int gpio_num, int num_leds, uint8_t brightness) {
    ...
    // Mutex first — must exist before any led_show() call so concurrent paths can't race.
    ...
    // 2.6.2 fix: keep the RMT channel alive even if this initial test fails.
    // A loose wire on boot used to permanently disable the strip until reflash.
    build_rmt_buf();
...
// One-shot blink — used by /api/v1/hub/identify, /api/v1/devices/<id>/identify,
// and the MQTT cmd/ident_hub + cmd/ident_tank handlers. Five fast white pulses
// at 6 Hz beat the regular 4 FPS render loop visually; we don't bother
// restoring afterwards because the next render-loop tick reclaims the slot.
static void led_identify_task(void *arg) {
    int led_idx = (int)(intptr_t)arg;
    for (int i = 0; i < 5; i++) {
        led_set(led_idx, LED_WHITE); led_show();
        vTaskDelay(pdMS_TO_TICKS(150));
        led_set(led_idx, LED_OFF);   led_show();
        vTaskDelay(pdMS_TO_TICKS(150));
    }
    vTaskDelete(NULL);
}

void led_identify(int led_idx) {
    xTaskCreate(led_identify_task, "led_ident", 2048,
                (void *)(intptr_t)led_idx, 5, NULL);
}
...
esp_err_t led_show(void) {
    ...
    // 2.6.2: if strip looks disconnected, back off to one retry every 5s rather than
    // burning 50ms per call. Original code permanently disabled after 6 fails.
    if (s_rmt_fail_count >= LED_FAIL_BACKOFF_THRESHOLD) {
        if (xTaskGetTickCount() - s_last_fail_tick < LED_RETRY_INTERVAL_TICKS) {
            if (s_led_mutex) xSemaphoreGive(s_led_mutex);
            return ESP_ERR_INVALID_STATE;
        }
        s_last_fail_tick = xTaskGetTickCount();  // mark this attempt
    }
    ...
    if (err == ESP_OK) {
        ...
        if (err != ESP_OK) {
            ...
            s_strip_present = false;
            if (s_rmt_fail_count == LED_FAIL_BACKOFF_THRESHOLD)
                ESP_LOGW(TAG, "RMT failing — strip likely disconnected. Backing off, will retry every 5s.");
        } else {
            if (s_rmt_fail_count > 0) {
                ESP_LOGI(TAG, "RMT recovered after %d failure(s) — strip is back.", s_rmt_fail_count);
            }
            s_rmt_fail_count = 0;
            s_strip_present = true;
        }
...
void led_get_status(led_status_t *out) {
    ...
    out->present     = s_strip_present;
    out->fail_count  = s_rmt_fail_count;
    out->num_leds    = s_num_leds;
    out->brightness  = s_brightness;
}

void led_reinit(int num_leds, uint8_t brightness) {
    if (num_leds < 1) num_leds = 1;
    if (num_leds > MAX_LEDS) num_leds = MAX_LEDS;

    int old_count = s_num_leds;

    // If shrinking, blank the soon-to-be-unused LEDs at the OLD frame size first
    // so trailing LEDs go dark cleanly before we shorten the frame.
    if (num_leds < old_count) {
        if (s_led_mutex) xSemaphoreTake(s_led_mutex, portMAX_DELAY);
        memset(s_leds, 0, sizeof(s_leds));
        if (s_led_mutex) xSemaphoreGive(s_led_mutex);
        led_show();  // sends a full old-size all-black frame
    }
    ...
    s_brightness = brightness;
    if (s_led_mutex) xSemaphoreGive(s_led_mutex);
    // Caller is expected to repaint via the registry/effect task, but a refresh
    // here ensures the new size + brightness shows up even if no other path runs.
    led_show();
}
