...
"<div class='card'><div class='card-title' style='display:flex;justify-content:space-between;align-items:center'><span id='editor-title'>EDIT DEVICE</span><button id='pair-btn' class='btn btn-sm' onclick='togglePairing()'>PAIR NEW</button></div>"
"<div id='tx-addr-row' style='margin-bottom:1.2rem;padding:.8rem;border:1px dashed #ffb000;display:flex;justify-content:space-between;align-items:center'>"
"<span style='opacity:0.7;font-size:.85rem'>LORA ADDRESS</span>"
"<span id='tx-addr-display' style='font-weight:bold;font-size:1.1rem'>#—</span>"
"<input type='number' id='tx-addr' style='display:none;width:100px;padding:.5rem;font-size:1rem'>"
"</div>"
"<div style='font-size:.7rem;opacity:.4;margin:-0.8rem 0 1.2rem'>ADDRESS IS ASSIGNED DURING PAIRING AND CANNOT BE CHANGED</div>"
"<div style='border:1px solid #ffb000;padding:1.2rem;margin-bottom:1.2rem'>"
"<div style='font-size:.8rem;opacity:.7;margin-bottom:1rem;border-bottom:1px dashed #ffb000;padding-bottom:.5rem'>— TANK INFO —</div>"
"<div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:1.5rem'>"
"<div><label>TANK NAME</label><input type='text' id='tx-name' maxlength='15'></div>"
"<div><label>CAPACITY (LITERS)</label><input type='number' id='tx-cap' min='1'></div>"
"</div></div>"
"<div style='border:1px solid #ffb000;padding:1.2rem;margin-bottom:1.2rem'>"
"<div style='font-size:.8rem;opacity:.7;margin-bottom:1rem;border-bottom:1px dashed #ffb000;padding-bottom:.5rem'>— SENSOR CALIBRATION —</div>"
"<div style='font-size:.7rem;opacity:.5;margin-bottom:1rem'>DISTANCE FROM ULTRASONIC SENSOR TO WATER SURFACE AT EACH LEVEL</div>"
"<div style='display:grid;grid-template-columns:1fr 1fr;gap:1.5rem'>"
"<div><label>DISTANCE WHEN FULL (CM)</label><input type='number' id='tx-min' min='2' max='400'>"
"<div style='font-size:.65rem;opacity:.4;margin-top:.3rem'>SENSOR → WATER (TANK FULL)</div></div>"
"<div><label>DISTANCE WHEN EMPTY (CM)</label><input type='number' id='tx-max' min='2' max='400'>"
"<div style='font-size:.65rem;opacity:.4;margin-top:.3rem'>SENSOR → BOTTOM (TANK EMPTY)</div></div>"
"</div></div>"
"<div style='border:1px solid #ffb000;padding:1.2rem;margin-bottom:1.5rem'>"
"<div style='font-size:.8rem;opacity:.7;margin-bottom:1rem;border-bottom:1px dashed #ffb000;padding-bottom:.5rem'>— TRANSMITTER (SENT VIA LORA) —</div>"
"<div style='font-size:.7rem;opacity:.5;margin-bottom:1rem'>CHANGES APPLY ON NEXT TRANSMITTER WAKE CYCLE</div>"
"<div style='display:grid;grid-template-columns:1fr 1fr;gap:1.5rem'>"
"<div><label>SLEEP INTERVAL (SEC)</label><input type='number' id='tx-sleep' min='60' max='86400'>"
"<div style='font-size:.65rem;opacity:.4;margin-top:.3rem'>60 – 86400 (1 MIN – 24 HR)</div></div>"
"<div><label>SENSOR SAMPLES</label><input type='number' id='tx-samp' min='3' max='20'>"
"<div style='font-size:.65rem;opacity:.4;margin-top:.3rem'>3 – 20 READINGS PER WAKE</div></div>"
"</div></div>"
"<button class='btn' onclick='saveTx()'>SAVE DEVICE</button></div>"
"<div class='card'><div class='card-title' style='display:flex;justify-content:space-between;align-items:center'><span>REGISTERED DEVICES</span><button class='btn btn-sm' style='border-color:#f00;color:#f00' onclick='clearAllTx()'>CLEAR ALL</button></div>"
"<div class='tx-table-container'><table class='tx-table'><thead><tr><th>ADDR</th><th>NAME</th><th>FULL</th><th>EMPTY</th><th>CAP</th><th>SLEEP</th><th>SAMP</th><th>STATE</th><th></th></tr></thead><tbody id='tx-tbody'></tbody></table></div></div></div>"
...
"let _editingAddr=0;"
"function editTx(a,n,mn,mx,c,s,sa){"
"_editingAddr=a;"
"document.getElementById('tx-addr').value=a;document.getElementById('tx-addr').style.display='none';"
"document.getElementById('tx-addr-display').textContent='#'+a;document.getElementById('tx-addr-display').style.display='';"
"document.getElementById('editor-title').textContent='EDIT DEVICE #'+a;"
"document.getElementById('tx-name').value=n;document.getElementById('tx-min').value=mn;document.getElementById('tx-max').value=mx;document.getElementById('tx-cap').value=c;document.getElementById('tx-sleep').value=s;document.getElementById('tx-samp').value=sa;window.scrollTo(0,0);}"
"async function saveTx(){"
"const addr=_editingAddr||+document.getElementById('tx-addr').value;"
"if(!addr||addr<1){showToast('INVALID ADDRESS',false);return;}"
"const sleep=+document.getElementById('tx-sleep').value;"
"const samp=+document.getElementById('tx-samp').value;"
"const mn=+document.getElementById('tx-min').value;"
"const mx=+document.getElementById('tx-max').value;"
"if(mn>=mx){showToast('FULL DISTANCE MUST BE LESS THAN EMPTY',false);return;}"
"if(sleep<60){showToast('SLEEP MUST BE >= 60 SECONDS',false);return;}"
"if(samp<3||samp>20){showToast('SAMPLES MUST BE 3-20',false);return;}"
"const b={addr:addr,name:document.getElementById('tx-name').value,min_dist:mn,max_dist:mx,capacity:+document.getElementById('tx-cap').value,sleep:sleep,samples:samp};"
"const r=await api('/api/transmitters',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});"
"showToast(r.ok===false?(r.message||'ERROR'):(sleep>=60?'SAVED — TX CONFIG QUEUED FOR NEXT WAKE':'SAVED'));loadTransmitters();}"
...
static esp_err_t handle_post_transmitters(httpd_req_t *req) {
    ...
    if (sleep >= 60) {
        registry_set_remote_config(addr, sleep, samples > 0 ? samples : 5);
        send_ok(req, "Saved — TX config queued");
    } else {
        send_ok(req, "Saved");
    }
    return ESP_OK;
